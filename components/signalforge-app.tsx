...
import { EnterpriseRoomControls } from '@/components/enterprise-room-controls';
import { WorkspaceRoster } from '@/components/workspace-roster';
import { BrandIcon } from '@/components/brand-icon';
import { UserAvatar } from '@/components/user-avatar';
import { DatadogDashboard } from '@/components/datadog-dashboard';
import {
  voiceAgentProfiles,
...
const nav: Array<{ id: View; label: string; icon: typeof Command }> = [
  ...
  { id: 'logs', label: 'Datadog dashboard', icon: Gauge },
...
function AgentTeam({
  ...
  workspaceRoom: WorkspaceContext['room'];
  connected: boolean;
  ...
  return (
    <section className="voice-team glass-card">
      ...
      <WorkspaceRoster room={workspaceRoom} />
    </section>
  );
}
    ...
    <div className="detail-view view-enter">
          ...
          return (
            <article className="integration-card glass-card" key={item.key}>
              <span className="integration-logo">
                <BrandIcon brand={item.key} size="lg" labelled />
              </span>
              <div>
                <h3>{item.name}</h3>
                <p>{item.detail}</p>
      ...
      <section className="action-console glass-card">
        <div className="action-console-copy">
          ...
          <p>
            Choose the reviewed view the team needs. Nothing is sent without
            explicit approval.
          </p>
        ...
        <div className="action-composer">
          <label>
            <span>
              Prepared {publishView} payload for {provider}
            </span>
            <textarea
              value={message ?? draft}
...
export default function SignalForgeApp({
  workspace,
}: {
  ...
  const [identity, setIdentity] = useState({
    ...
    role:
      workspace.room.role === 'owner'
        ? 'Incident commander'
        : workspace.room.role,
  });
  const [title, setTitle] = useState(workspace.room.name);
  const [severity, setSeverity] = useState(workspace.room.severity);
  ...
  const room = useAgoraRoom(appId, identity, workspace.room.slug);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => void nextDemoLine(), demoDelay);
    return () => window.clearTimeout(timer);
  }, [demoDelay, demoStatus, demoStep, nextDemoLine]);
  useEffect(() => {
    if (!Object.values(agents).includes('running') || demoStatus !== 'idle')
      return;
    let active = true;
  ...
  return (
    <main className="app-shell">
      ...
      <aside className="sidebar glass-panel">
        ...
        <EnterpriseRoomControls
          room={workspace.room}
          organizationName={workspace.user.organizationName}
        />
        ...
        <button
          ...
          aria-label={`Open account menu for ${identity.name || 'your profile'}`}
        >
          <UserAvatar
            name={identity.name || 'Responder'}
            identity={workspace.user.email}
            size="sm"
          />
          <div>
        ...
        {profileOpen && (
          <div className="profile-menu">
            <div>
              <b>{workspace.user.email}</b>
              <small>{workspace.user.organizationRole}</small>
            </div>
            <a href="/rooms">
              <Users size={15} />
              All rooms
                    ...
                    <div className="live-panel-heading">
                      ...
                      <b>
                        {latest?.engine === 'gemini' && (
                          <BrandIcon brand="gemini" size="sm" />
                        )}
                        {latest?.engine === 'gemini'
                          ? 'Gemini live'
                          : 'Listening'}
                    ...
                    {!speech.interim &&
                      ...
                      analyzing && (
                        <div className="classification-pending">
                          ...
                          <span>
                            Gemini is separating evidence, uncertainty, and next
                            actions…
                          </span>
                        </div>
                      )}
            ...
            <section className="metric-section glass-card">
              <div className="section-heading">
                <div>
                  ...
                  <h2 className="branded-heading">
                    <BrandIcon brand="datadog" size="sm" />
                    Current Datadog signals
                  </h2>
                </div>
